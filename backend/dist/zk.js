...
class Zokrates {
    ...
    generateProof(input) {
        ...
        // generate proof
        return this.provider.generateProof(this.artifacts.program, witness, this.keypair.pk);
    }
    verifyProof(proof) {
...
(async () => {
    ...
    const zkSourceData = readFileSync(zkSource);
    const secretsData = readFileSync(secrets);
    const secretsJson = JSON.parse(secretsData.toString());
